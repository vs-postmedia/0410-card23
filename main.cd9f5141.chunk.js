...
var web_dom_collections_for_each = __webpack_require__(116);

// EXTERNAL MODULE: ./node_modules/core-js/modules/es.object.get-own-property-descriptors.js
var es_object_get_own_property_descriptors = __webpack_require__(120);
...
var d3_scale_chromatic_src = __webpack_require__(87);
...
var d3_collection_src = __webpack_require__(88);

// EXTERNAL MODULE: ./node_modules/@flourish/popup/src/index.js + 5 modules
var popup_src = __webpack_require__(89);

// EXTERNAL MODULE: ./node_modules/d3-svg-annotation/indexRollupNext.js + 108 modules
var indexRollupNext = __webpack_require__(85);

// EXTERNAL MODULE: ./src/js/TooltipTemplate/tooltip-template.css
var tooltip_template = __webpack_require__(128);
...
var calendar_heatmap = __webpack_require__(129);
...
var calendar_heatmap_d3 = _objectSpread(_objectSpread(_objectSpread(_objectSpread(_objectSpread(_objectSpread(_objectSpread({}, d3), d3_time_src), d3_time_format_src), d3_collection_src), src), d3_scale_chromatic_src), indexRollupNext);
...
var drawLegend = function drawLegend(svg) {
  calendar_heatmap_d3.select('#calendar').append('div').attr('id', 'legend').append('text').text('Calls per day'); // .style('transform', 'translate(200, 50)')
};

function handleMouseenter(d) {
  var date = dateParser(d);
  calendar_heatmap_d3.select(this).classed('hover', true);
  popup.point([event.x, event.y - 5]).html(TooltipTemplate_tooltip_template(displayFormat(date), lookup[d])).draw();
}
...
var drawCalendar = function drawCalendar(data, annotations, highlights) {
  ...
  var svg = calendar_heatmap_d3.select('#calendar').append('svg').attr('height', 1800);
  var g = svg.selectAll('g').data(months).enter().append('g').attr('class', 'month').attr('id', function (d) {
    return "".concat(monthName(d).toLowerCase());
  }).attr('width', cellSize * 7 + cellMargin * 8).attr('height', function (d) {
    var rows = calendarRows(d);
    return cellSize * rows + cellMargin * (rows + 1) + monthLabelHeight;
  }).attr('transform', function (d, i, elem) {
    var rows = calendarRows(d);
    var height = cellSize * rows + cellMargin * (rows + 1) + monthLabelHeight + monthMargin;
    yOffset += height;
    return "translate(0, ".concat(yOffset - height, ")");
  });
  g.append('text').attr('class', 'month-name').attr('x', (cellSize * 7 + cellMargin * 8) / 2).attr('y', 15).attr('text-anchor', 'middle').text(function (d) {
    return monthName(d);
  });
  var rect = g.selectAll('rect.day').data(function (d, i) {
  ...
  }).on('mouseover', handleMouseenter).on('mouseout', handleMouseout).datum(format); // colour calendar days

  rect.style('fill', function (d) {
    return calendar_heatmap_d3.interpolatePuBu(scale(lookup[d]));
  }).attr('class', function (d) {
    return highlights.includes(d) ? 'day ia-anno' : 'day';
  }); // draw legend
...
var card23_data = __webpack_require__(82);

// EXTERNAL MODULE: ./src/data/annotations.json
var annotations = __webpack_require__(83);

// EXTERNAL MODULE: ./src/data/card23-highlights.json
var card23_highlights = __webpack_require__(84);
    ...
    return regenerator_default.a.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            calendar_heatmap_calendar_heatmap.drawCalendar(card23_data, annotations, card23_highlights);
...
module.exports = JSON.parse("[{\"note\":{\"label\":\"anno-01 label\",\"title\":\"anno-01 title\"},\"x\":112,\"y\":88,\"dy\":10,\"dx\":10,\"subject\":{\"radius\":50,\"radiusPadding\":10}}]");
